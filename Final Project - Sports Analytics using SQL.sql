...
select bowler,max(extra_runs) as Max_runs_conced 
from deliveries group by bowler 
order by max(extra_runs)
desc limit 5;
...
select extract(year from date) as year_runs,total_runs
from deliveries_v03 
where venue='Eden Gardens'
group by total_runs,year_runs
...
select team1_corr from matches_corrected group by team1_corr;
